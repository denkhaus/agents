...
import (
	...
	"github.com/denkhaus/project-manager/internal/types"
	"github.com/google/uuid"
)

// memoryRepository implements Repository interface with in-memory storage using resource managers
type memoryRepository struct {
	projects       *resource.Manager[*types.Project]
	tasks          *resource.Manager[*types.Task]
...
func NewMemoryRepository() types.Repository {
	return &memoryRepository{
		projects:       resource.NewManager[*types.Project](),
		tasks:          resource.NewManager[*types.Task](),
...
func (r *memoryRepository) CreateProject(ctx context.Context, project *types.Project) error {
...
func (r *memoryRepository) UpdateProject(ctx context.Context, project *types.Project) error {
	return r.projects.UpdateWithError(project.ID, func(existing *types.Project) (*types.Project, error) {
		projectCopy := *project
...
func (r *memoryRepository) ListProjects(ctx context.Context) ([]*types.Project, error) {
	allProjects := r.projects.GetAll()
	projects := make([]*types.Project, 0, len(allProjects))
...
func (r *memoryRepository) UpdateTask(ctx context.Context, task *types.Task) error {
	...
	err := r.tasks.UpdateWithError(task.ID, func(existing *types.Task) (*types.Task, error) {
		taskCopy := *task
		...
		if task.State == types.TaskStateCompleted && existing.State != types.TaskStateCompleted {
			now := time.Now()
...
func (r *memoryRepository) updateProjectMetrics(projectID uuid.UUID) {
	r.projects.Update(projectID, func(project *types.Project) *types.Project {
		...
		for _, taskID := range taskIDs {
			if task, exists := r.tasks.Get(taskID); exists && task.State == types.TaskStateCompleted {
				completedTasks++
			}
		}
...
func (r *memoryRepository) matchesFilter(task *types.Task, filter types.TaskFilter) bool {
...
func (r *memoryRepository) GetTasksByProject(ctx context.Context, projectID uuid.UUID) ([]*types.Task, error) {
	return r.ListTasks(ctx, types.TaskFilter{ProjectID: &projectID})
}

func (r *memoryRepository) GetTasksByParent(ctx context.Context, parentID uuid.UUID) ([]*types.Task, error) {
	childIDs, exists := r.tasksByParent.Get(parentID)
	if !exists {
		return []*types.Task{}, nil
	}

	var childTasks []*types.Task
...
func (r *memoryRepository) GetRootTasks(ctx context.Context, projectID uuid.UUID) ([]*types.Task, error) {
	taskIDs, exists := r.tasksByProject.Get(projectID)
	if !exists {
		return []*types.Task{}, nil
	}
...
func (r *memoryRepository) GetParentTask(ctx context.Context, taskID uuid.UUID) (*types.Task, error) {
...
func (r *memoryRepository) GetProjectProgress(ctx context.Context, projectID uuid.UUID) (*types.ProjectProgress, error) {
...
func (r *memoryRepository) AddTaskDependency(ctx context.Context, taskID uuid.UUID, dependsOnTaskID uuid.UUID) (*types.Task, error) {
...
func (r *memoryRepository) RemoveTaskDependency(ctx context.Context, taskID uuid.UUID, dependsOnTaskID uuid.UUID) (*types.Task, error) {
	task, err := r.GetTask(ctx, taskID)
	if err != nil {
		return nil, err
	}
...
func (r *memoryRepository) GetTaskDependencies(ctx context.Context, taskID uuid.UUID) ([]*types.Task, error) {
	task, err := r.GetTask(ctx, taskID)
	if err != nil {
		return nil, err
	}

	dependencies := make([]*types.Task, 0, len(task.Dependencies))
	for _, depID := range task.Dependencies {
...
func (r *memoryRepository) GetDependentTasks(ctx context.Context, taskID uuid.UUID) ([]*types.Task, error) {
	allTasks := r.tasks.GetAll()
	dependents := make([]*types.Task, 0)
