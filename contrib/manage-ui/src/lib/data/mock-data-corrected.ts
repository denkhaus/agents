import type { Project, Task } from '../types';

// Helper function to generate realistic dates
const getRandomDate = (daysAgo: number) => {
  const date = new Date();
  date.setDate(date.getDate() - Math.floor(Math.random() * daysAgo));
  return date.toISOString();
};

// Pre-generated UUIDs for consistent references (following Go schema strictly)
const PROJECT_IDS = {
  frontend: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d',
  backend: 'b2c3d4e5-f6a7-4b8c-9d0e-1f2a3b4c5d6e',
  orchestration: 'c3d4e5f6-a7b8-4c9d-0e1f-2a3b4c5d6e7f'
};

const AGENT_IDS = {
  frontend1: 'd4e5f6a7-b8c9-4d0e-1f2a-3b4c5d6e7f8a',
  frontend2: 'e5f6a7b8-c9d0-4e1f-2a3b-4c5d6e7f8a9b',
  design1: 'f6a7b8c9-d0e1-4f2a-3b4c-5d6e7f8a9b0c',
  backend1: 'a7b8c9d0-e1f2-4a3b-4c5d-6e7f8a9b0c1d',
  backend2: 'b8c9d0e1-f2a3-4b4c-5d6e-7f8a9b0c1d2e',
  ai1: 'c9d0e1f2-a3b4-4c5d-6e7f-8a9b0c1d2e3f',
  ai2: 'd0e1f2a3-b4c5-4d6e-7f8a-9b0c1d2e3f4a'
};

// Mock Projects (strictly following Go schema)
export const mockProjects: Project[] = [
  {
    id: PROJECT_IDS.frontend,
    title: 'Multi-Agent System Frontend',
    description: 'Entwicklung einer modernen Admin-Oberfläche für das Multi-Agent System mit NextJS, TypeScript und Tailwind CSS.',
    created_at: getRandomDate(30),
    updated_at: getRandomDate(2),
    total_tasks: 15,
    completed_tasks: 5,
    progress: 33.33
  },
  {
    id: PROJECT_IDS.backend,
    title: 'Backend API Enhancement',
    description: 'Verbesserung und Erweiterung der Backend-API für bessere Performance und neue Features.',
    created_at: getRandomDate(45),
    updated_at: getRandomDate(1),
    total_tasks: 12,
    completed_tasks: 9,
    progress: 75.0
  },
  {
    id: PROJECT_IDS.orchestration,
    title: 'AI Agent Orchestration Platform',
    description: 'Entwicklung einer Plattform zur Orchestrierung und Verwaltung von KI-Agenten.',
    created_at: getRandomDate(60),
    updated_at: getRandomDate(3),
    total_tasks: 18,
    completed_tasks: 7,
    progress: 38.89
  }
];

// Pre-generated Task UUIDs (strictly following Go schema)
const TASK_IDS = {
  // Project 1: Frontend - Root Tasks
  p1_setup: 'e1f2a3b4-c5d6-4e7f-8a9b-0c1d2e3f4a5b',
  p1_design: 'f2a3b4c5-d6e7-4f8a-9b0c-1d2e3f4a5b6c',
  p1_state: 'a3b4c5d6-e7f8-4a9b-0c1d-2e3f4a5b6c7d',
  p1_kanban: 'b4c5d6e7-f8a9-4b0c-1d2e-3f4a5b6c7d8e',
  p1_chat: 'c5d6e7f8-a9b0-4c1d-2e3f-4a5b6c7d8e9f',
  
  // Project 1: Frontend - Subtasks
  p1_setup_nextjs: 'd6e7f8a9-b0c1-4d2e-3f4a-5b6c7d8e9f0a',
  p1_setup_tailwind: 'e7f8a9b0-c1d2-4e3f-4a5b-6c7d8e9f0a1b',
  p1_setup_structure: 'f8a9b0c1-d2e3-4f4a-5b6c-7d8e9f0a1b2c',
  p1_design_shadcn: 'a9b0c1d2-e3f4-4a5b-6c7d-8e9f0a1b2c3d',
  p1_design_layout: 'b0c1d2e3-f4a5-4b6c-7d8e-9f0a1b2c3d4e',
  p1_design_theme: 'c1d2e3f4-a5b6-4c7d-8e9f-0a1b2c3d4e5f',
  p1_state_stores: 'd2e3f4a5-b6c7-4d8e-9f0a-1b2c3d4e5f6a',
  p1_state_api: 'e3f4a5b6-c7d8-4e9f-0a1b-2c3d4e5f6a7b',
  p1_kanban_layout: 'f4a5b6c7-d8e9-4f0a-1b2c-3d4e5f6a7b8c',
  
  // Project 2: Backend - Root Tasks  
  p2_performance: 'a5b6c7d8-e9f0-4a1b-2c3d-4e5f6a7b8c9d',
  p2_sse: 'b6c7d8e9-f0a1-4b2c-3d4e-5f6a7b8c9d0e',
  p2_chat_api: 'c7d8e9f0-a1b2-4c3d-4e5f-6a7b8c9d0e1f',
  p2_migration: 'd8e9f0a1-b2c3-4d4e-5f6a-7b8c9d0e1f2a',
  p2_docs: 'e9f0a1b2-c3d4-4e5f-6a7b-8c9d0e1f2a3b',
  
  // Project 2: Backend - Subtasks
  p2_perf_db: 'f0a1b2c3-d4e5-4f6a-7b8c-9d0e1f2a3b4c',
  p2_perf_cache: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d',
  
  // Project 3: AI Orchestration - Root Tasks
  p3_lifecycle: 'b2c3d4e5-f6a7-4b8c-9d0e-1f2a3b4c5d6e',
  p3_assignment: 'c3d4e5f6-a7b8-4c9d-0e1f-2a3b4c5d6e7f',
  p3_monitoring: 'd4e5f6a7-b8c9-4d0e-1f2a-3b4c5d6e7f8a',
  p3_communication: 'e5f6a7b8-c9d0-4e1f-2a3b-4c5d6e7f8a9b',
  p3_security: 'f6a7b8c9-d0e1-4f2a-3b4c-5d6e7f8a9b0c',
  p3_loadbalancing: 'a7b8c9d0-e1f2-4a3b-4c5d-6e7f8a9b0c1d',
  
  // Project 3: AI Orchestration - Subtasks
  p3_lifecycle_registry: 'b8c9d0e1-f2a3-4b4c-5d6e-7f8a9b0c1d2e',
  p3_lifecycle_health: 'c9d0e1f2-a3b4-4c5d-6e7f-8a9b0c1d2e3f',
  p3_lifecycle_scaling: 'd0e1f2a3-b4c5-4d6e-7f8a-9b0c1d2e3f4a',
  p3_assignment_matching: 'e1f2a3b4-c5d6-4e7f-8a9b-0c1d2e3f4a5b',
  p3_assignment_queue: 'f2a3b4c5-d6e7-4f8a-9b0c-1d2e3f4a5b6c',
  p3_monitoring_metrics: 'a3b4c5d6-e7f8-4a9b-0c1d-2e3f4a5b6c7d',
  p3_monitoring_dashboard: 'b4c5d6e7-f8a9-4b0c-1d2e-3f4a5b6c7d8e'
};

// Mock Tasks (strictly following Go schema with proper UUIDs)
export const mockTasks: Task[] = [
  // PROJECT 1: Multi-Agent System Frontend
  // Root Tasks (depth 0)
  {
    id: TASK_IDS.p1_setup,
    project_id: PROJECT_IDS.frontend,
    parent_id: null,
    title: 'Projekt Setup und Architektur',
    description: 'Initialisierung des NextJS Projekts, Setup der Entwicklungsumgebung und Definition der Grundarchitektur.',
    state: 'completed',
    complexity: 5,
    depth: 0,
    estimate: 480,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [],
    dependents: [TASK_IDS.p1_design, TASK_IDS.p1_state],
    created_at: getRandomDate(25),
    updated_at: getRandomDate(20),
    completed_at: getRandomDate(20)
  },
  {
    id: TASK_IDS.p1_design,
    project_id: PROJECT_IDS.frontend,
    parent_id: null,
    title: 'UI/UX Design System',
    description: 'Entwicklung eines konsistenten Design Systems mit Tailwind CSS und shadcn/ui Komponenten.',
    state: 'completed',
    complexity: 7,
    depth: 0,
    estimate: 720,
    assigned_agent: AGENT_IDS.design1,
    dependencies: [TASK_IDS.p1_setup],
    dependents: [TASK_IDS.p1_kanban],
    created_at: getRandomDate(22),
    updated_at: getRandomDate(15),
    completed_at: getRandomDate(15)
  },
  {
    id: TASK_IDS.p1_state,
    project_id: PROJECT_IDS.frontend,
    parent_id: null,
    title: 'State Management Implementation',
    description: 'Implementierung von Zustand für globales State Management und API-Integration.',
    state: 'in-progress',
    complexity: 8,
    depth: 0,
    estimate: 600,
    assigned_agent: AGENT_IDS.frontend2,
    dependencies: [TASK_IDS.p1_setup],
    dependents: [TASK_IDS.p1_chat],
    created_at: getRandomDate(20),
    updated_at: getRandomDate(2)
  },
  {
    id: TASK_IDS.p1_kanban,
    project_id: PROJECT_IDS.frontend,
    parent_id: null,
    title: 'Kanban Task Management',
    description: 'Entwicklung der hierarchischen Kanban-Ansicht für Task-Management.',
    state: 'in-progress',
    complexity: 9,
    depth: 0,
    estimate: 960,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [TASK_IDS.p1_design],
    dependents: [],
    created_at: getRandomDate(18),
    updated_at: getRandomDate(1)
  },
  {
    id: TASK_IDS.p1_chat,
    project_id: PROJECT_IDS.frontend,
    parent_id: null,
    title: 'Chat Integration',
    description: 'Integration der LLM-Chat-Funktionalität für Task- und Projekt-Assistenz.',
    state: 'pending',
    complexity: 8,
    depth: 0,
    estimate: 720,
    assigned_agent: null,
    dependencies: [TASK_IDS.p1_state],
    dependents: [],
    created_at: getRandomDate(15),
    updated_at: getRandomDate(15)
  },

  // Subtasks for Project Setup (depth 1)
  {
    id: TASK_IDS.p1_setup_nextjs,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_setup,
    title: 'NextJS Projekt initialisieren',
    description: 'Setup des NextJS 15 Projekts mit TypeScript und App Router.',
    state: 'completed',
    complexity: 3,
    depth: 1,
    estimate: 120,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [],
    dependents: [TASK_IDS.p1_setup_tailwind],
    created_at: getRandomDate(25),
    updated_at: getRandomDate(24),
    completed_at: getRandomDate(24)
  },
  {
    id: TASK_IDS.p1_setup_tailwind,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_setup,
    title: 'Tailwind CSS v4 konfigurieren',
    description: 'Installation und Konfiguration von Tailwind CSS v4 mit CSS-Variablen.',
    state: 'completed',
    complexity: 4,
    depth: 1,
    estimate: 180,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [TASK_IDS.p1_setup_nextjs],
    dependents: [TASK_IDS.p1_setup_structure],
    created_at: getRandomDate(24),
    updated_at: getRandomDate(23),
    completed_at: getRandomDate(23)
  },
  {
    id: TASK_IDS.p1_setup_structure,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_setup,
    title: 'Projektstruktur definieren',
    description: 'Erstellung der Ordnerstruktur und Definition der Architektur-Patterns.',
    state: 'completed',
    complexity: 3,
    depth: 1,
    estimate: 180,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [TASK_IDS.p1_setup_tailwind],
    dependents: [],
    created_at: getRandomDate(23),
    updated_at: getRandomDate(22),
    completed_at: getRandomDate(22)
  },

  // Subtasks for Design System (depth 1)
  {
    id: TASK_IDS.p1_design_shadcn,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_design,
    title: 'shadcn/ui Setup',
    description: 'Installation und Konfiguration der shadcn/ui Komponentenbibliothek.',
    state: 'completed',
    complexity: 4,
    depth: 1,
    estimate: 240,
    assigned_agent: AGENT_IDS.design1,
    dependencies: [],
    dependents: [TASK_IDS.p1_design_layout],
    created_at: getRandomDate(22),
    updated_at: getRandomDate(18),
    completed_at: getRandomDate(18)
  },
  {
    id: TASK_IDS.p1_design_layout,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_design,
    title: 'Layout Komponenten',
    description: 'Entwicklung der Basis-Layout-Komponenten (Sidebar, Navigation, etc.).',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 360,
    assigned_agent: AGENT_IDS.design1,
    dependencies: [TASK_IDS.p1_design_shadcn],
    dependents: [TASK_IDS.p1_design_theme],
    created_at: getRandomDate(20),
    updated_at: getRandomDate(16),
    completed_at: getRandomDate(16)
  },
  {
    id: TASK_IDS.p1_design_theme,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_design,
    title: 'Theme System',
    description: 'Implementierung des Dark/Light Theme Systems mit CSS-Variablen.',
    state: 'completed',
    complexity: 5,
    depth: 1,
    estimate: 120,
    assigned_agent: AGENT_IDS.design1,
    dependencies: [TASK_IDS.p1_design_layout],
    dependents: [],
    created_at: getRandomDate(18),
    updated_at: getRandomDate(15),
    completed_at: getRandomDate(15)
  },

  // Subtasks for State Management (depth 1)
  {
    id: TASK_IDS.p1_state_stores,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_state,
    title: 'Zustand Stores definieren',
    description: 'Definition der Zustand Stores für Project, UI und Chat Management.',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 300,
    assigned_agent: AGENT_IDS.frontend2,
    dependencies: [],
    dependents: [TASK_IDS.p1_state_api],
    created_at: getRandomDate(20),
    updated_at: getRandomDate(10),
    completed_at: getRandomDate(10)
  },
  {
    id: TASK_IDS.p1_state_api,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_state,
    title: 'API Client implementieren',
    description: 'Entwicklung des API Clients mit Error Handling und TypeScript Types.',
    state: 'in-progress',
    complexity: 7,
    depth: 1,
    estimate: 300,
    assigned_agent: AGENT_IDS.frontend2,
    dependencies: [TASK_IDS.p1_state_stores],
    dependents: [],
    created_at: getRandomDate(15),
    updated_at: getRandomDate(2)
  },

  // Subtasks for Kanban (depth 1)
  {
    id: TASK_IDS.p1_kanban_layout,
    project_id: PROJECT_IDS.frontend,
    parent_id: TASK_IDS.p1_kanban,
    title: 'Kanban Board Layout',
    description: 'Grundlegendes Layout für die Kanban-Ansicht mit Spalten und Cards.',
    state: 'in-progress',
    complexity: 6,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.frontend1,
    dependencies: [],
    dependents: [],
    created_at: getRandomDate(18),
    updated_at: getRandomDate(1)
  },

  // PROJECT 2: Backend API Enhancement
  // Root Tasks (depth 0)
  {
    id: TASK_IDS.p2_performance,
    project_id: PROJECT_IDS.backend,
    parent_id: null,
    title: 'API Performance Optimierung',
    description: 'Optimierung der bestehenden API-Endpunkte für bessere Performance und Skalierbarkeit.',
    state: 'completed',
    complexity: 8,
    depth: 0,
    estimate: 960,
    assigned_agent: AGENT_IDS.backend1,
    dependencies: [],
    dependents: [TASK_IDS.p2_sse],
    created_at: getRandomDate(40),
    updated_at: getRandomDate(25),
    completed_at: getRandomDate(25)
  },
  {
    id: TASK_IDS.p2_sse,
    project_id: PROJECT_IDS.backend,
    parent_id: null,
    title: 'SSE Implementation',
    description: 'Implementierung von Server-Sent Events für Real-time Updates.',
    state: 'completed',
    complexity: 7,
    depth: 0,
    estimate: 720,
    assigned_agent: AGENT_IDS.backend2,
    dependencies: [TASK_IDS.p2_performance],
    dependents: [TASK_IDS.p2_migration],
    created_at: getRandomDate(35),
    updated_at: getRandomDate(20),
    completed_at: getRandomDate(20)
  },
  {
    id: TASK_IDS.p2_chat_api,
    project_id: PROJECT_IDS.backend,
    parent_id: null,
    title: 'Chat API Integration',
    description: 'Entwicklung der Chat-API für LLM-Integration und Session-Management.',
    state: 'completed',
    complexity: 9,
    depth: 0,
    estimate: 1200,
    assigned_agent: AGENT_IDS.backend1,
    dependencies: [],
    dependents: [TASK_IDS.p2_docs],
    created_at: getRandomDate(30),
    updated_at: getRandomDate(15),
    completed_at: getRandomDate(15)
  },
  {
    id: TASK_IDS.p2_migration,
    project_id: PROJECT_IDS.backend,
    parent_id: null,
    title: 'Database Schema Migration',
    description: 'Migration der Datenbankschemas für neue Features und Performance-Verbesserungen.',
    state: 'in-progress',
    complexity: 6,
    depth: 0,
    estimate: 480,
    assigned_agent: AGENT_IDS.backend2,
    dependencies: [TASK_IDS.p2_sse],
    dependents: [],
    created_at: getRandomDate(25),
    updated_at: getRandomDate(3)
  },
  {
    id: TASK_IDS.p2_docs,
    project_id: PROJECT_IDS.backend,
    parent_id: null,
    title: 'API Documentation Update',
    description: 'Aktualisierung der API-Dokumentation mit OpenAPI/Swagger Specs.',
    state: 'pending',
    complexity: 4,
    depth: 0,
    estimate: 360,
    assigned_agent: AGENT_IDS.backend1,
    dependencies: [TASK_IDS.p2_chat_api],
    dependents: [],
    created_at: getRandomDate(20),
    updated_at: getRandomDate(20)
  },

  // Subtasks for Performance (depth 1)
  {
    id: TASK_IDS.p2_perf_db,
    project_id: PROJECT_IDS.backend,
    parent_id: TASK_IDS.p2_performance,
    title: 'Database Query Optimierung',
    description: 'Optimierung der Datenbankabfragen und Indizierung.',
    state: 'completed',
    complexity: 7,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.backend1,
    dependencies: [],
    dependents: [TASK_IDS.p2_perf_cache],
    created_at: getRandomDate(40),
    updated_at: getRandomDate(30),
    completed_at: getRandomDate(30)
  },
  {
    id: TASK_IDS.p2_perf_cache,
    project_id: PROJECT_IDS.backend,
    parent_id: TASK_IDS.p2_performance,
    title: 'Caching Strategy',
    description: 'Implementierung einer effizienten Caching-Strategie.',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 360,
    assigned_agent: AGENT_IDS.backend1,
    dependencies: [TASK_IDS.p2_perf_db],
    dependents: [],
    created_at: getRandomDate(35),
    updated_at: getRandomDate(25),
    completed_at: getRandomDate(25)
  },

  // PROJECT 3: AI Agent Orchestration Platform
  // Root Tasks (depth 0)
  {
    id: TASK_IDS.p3_lifecycle,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Agent Lifecycle Management',
    description: 'Entwicklung des Systems für Agent-Erstellung, -Verwaltung und -Überwachung.',
    state: 'completed',
    complexity: 9,
    depth: 0,
    estimate: 1440,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [],
    dependents: [TASK_IDS.p3_assignment, TASK_IDS.p3_monitoring],
    created_at: getRandomDate(55),
    updated_at: getRandomDate(35),
    completed_at: getRandomDate(35)
  },
  {
    id: TASK_IDS.p3_assignment,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Task Assignment Engine',
    description: 'Intelligente Task-Zuweisung basierend auf Agent-Fähigkeiten und Verfügbarkeit.',
    state: 'completed',
    complexity: 8,
    depth: 0,
    estimate: 1200,
    assigned_agent: AGENT_IDS.ai2,
    dependencies: [TASK_IDS.p3_lifecycle],
    dependents: [TASK_IDS.p3_communication],
    created_at: getRandomDate(50),
    updated_at: getRandomDate(30),
    completed_at: getRandomDate(30)
  },
  {
    id: TASK_IDS.p3_monitoring,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Performance Monitoring',
    description: 'Real-time Monitoring und Analytics für Agent-Performance.',
    state: 'in-progress',
    complexity: 7,
    depth: 0,
    estimate: 960,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [TASK_IDS.p3_lifecycle],
    dependents: [],
    created_at: getRandomDate(45),
    updated_at: getRandomDate(5)
  },
  {
    id: TASK_IDS.p3_communication,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Inter-Agent Communication',
    description: 'Protokoll und Infrastruktur für Kommunikation zwischen Agenten.',
    state: 'in-progress',
    complexity: 9,
    depth: 0,
    estimate: 1440,
    assigned_agent: AGENT_IDS.ai2,
    dependencies: [TASK_IDS.p3_assignment],
    dependents: [TASK_IDS.p3_security],
    created_at: getRandomDate(40),
    updated_at: getRandomDate(2)
  },
  {
    id: TASK_IDS.p3_security,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Security Framework',
    description: 'Sicherheitsframework für Agent-Authentifizierung und -Autorisierung.',
    state: 'pending',
    complexity: 8,
    depth: 0,
    estimate: 960,
    assigned_agent: null,
    dependencies: [TASK_IDS.p3_communication],
    dependents: [],
    created_at: getRandomDate(35),
    updated_at: getRandomDate(35)
  },
  {
    id: TASK_IDS.p3_loadbalancing,
    project_id: PROJECT_IDS.orchestration,
    parent_id: null,
    title: 'Load Balancing',
    description: 'Implementierung von Load Balancing für Agent-Workloads.',
    state: 'blocked',
    complexity: 7,
    depth: 0,
    estimate: 720,
    assigned_agent: null,
    dependencies: [],
    dependents: [],
    created_at: getRandomDate(30),
    updated_at: getRandomDate(10)
  },

  // Subtasks for Agent Lifecycle (depth 1)
  {
    id: TASK_IDS.p3_lifecycle_registry,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_lifecycle,
    title: 'Agent Registry',
    description: 'Zentrales Registry für alle verfügbaren Agenten und ihre Capabilities.',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [],
    dependents: [TASK_IDS.p3_lifecycle_health],
    created_at: getRandomDate(55),
    updated_at: getRandomDate(45),
    completed_at: getRandomDate(45)
  },
  {
    id: TASK_IDS.p3_lifecycle_health,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_lifecycle,
    title: 'Agent Health Monitoring',
    description: 'System zur Überwachung des Gesundheitszustands von Agenten.',
    state: 'completed',
    complexity: 7,
    depth: 1,
    estimate: 600,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [TASK_IDS.p3_lifecycle_registry],
    dependents: [TASK_IDS.p3_lifecycle_scaling],
    created_at: getRandomDate(50),
    updated_at: getRandomDate(40),
    completed_at: getRandomDate(40)
  },
  {
    id: TASK_IDS.p3_lifecycle_scaling,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_lifecycle,
    title: 'Auto-scaling Logic',
    description: 'Automatische Skalierung von Agenten basierend auf Workload.',
    state: 'completed',
    complexity: 8,
    depth: 1,
    estimate: 360,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [TASK_IDS.p3_lifecycle_health],
    dependents: [],
    created_at: getRandomDate(45),
    updated_at: getRandomDate(35),
    completed_at: getRandomDate(35)
  },

  // Subtasks for Task Assignment (depth 1)
  {
    id: TASK_IDS.p3_assignment_matching,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_assignment,
    title: 'Capability Matching Algorithm',
    description: 'Algorithmus zum Matching von Tasks mit Agent-Capabilities.',
    state: 'completed',
    complexity: 8,
    depth: 1,
    estimate: 720,
    assigned_agent: AGENT_IDS.ai2,
    dependencies: [],
    dependents: [TASK_IDS.p3_assignment_queue],
    created_at: getRandomDate(50),
    updated_at: getRandomDate(35),
    completed_at: getRandomDate(35)
  },
  {
    id: TASK_IDS.p3_assignment_queue,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_assignment,
    title: 'Priority Queue System',
    description: 'Prioritäts-basiertes Queue-System für Task-Assignment.',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.ai2,
    dependencies: [TASK_IDS.p3_assignment_matching],
    dependents: [],
    created_at: getRandomDate(45),
    updated_at: getRandomDate(30),
    completed_at: getRandomDate(30)
  },

  // Subtasks for Performance Monitoring (depth 1)
  {
    id: TASK_IDS.p3_monitoring_metrics,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_monitoring,
    title: 'Metrics Collection',
    description: 'System zur Sammlung von Performance-Metriken von Agenten.',
    state: 'completed',
    complexity: 6,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [],
    dependents: [TASK_IDS.p3_monitoring_dashboard],
    created_at: getRandomDate(45),
    updated_at: getRandomDate(25),
    completed_at: getRandomDate(25)
  },
  {
    id: TASK_IDS.p3_monitoring_dashboard,
    project_id: PROJECT_IDS.orchestration,
    parent_id: TASK_IDS.p3_monitoring,
    title: 'Dashboard Implementation',
    description: 'Real-time Dashboard für Performance-Monitoring.',
    state: 'in-progress',
    complexity: 7,
    depth: 1,
    estimate: 480,
    assigned_agent: AGENT_IDS.ai1,
    dependencies: [TASK_IDS.p3_monitoring_metrics],
    dependents: [],
    created_at: getRandomDate(30),
    updated_at: getRandomDate(5)
  }
];

// Calculate and update project progress based on tasks
export const updateProjectProgress = () => {
  mockProjects.forEach(project => {
    const projectTasks = mockTasks.filter(task => task.project_id === project.id);
    const completedTasks = projectTasks.filter(task => task.state === 'completed');
    
    project.total_tasks = projectTasks.length;
    project.completed_tasks = completedTasks.length;
    project.progress = projectTasks.length > 0 ? (completedTasks.length / projectTasks.length) * 100 : 0;
  });
};

// Initialize project progress
updateProjectProgress();