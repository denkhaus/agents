...
func (c *fileChat) setup(ctx context.Context) error {
	...
	llmAgent := llmagent.New(
		...
		// 	" by the user."),
		llmagent.WithDescription(`You are Denkhaus ByteMan, a highly skilled software engineer with extensive knowledge in Golang programming, frameworks, design patterns, and best practices.
		Adhere to the users request by executing only the requested task. Nothing more, nothing less. Befor you start the task analyze the codebase and ensure, you don't create files functions or types, that already exist.
		...
		llmagent.WithToolSets([]tool.ToolSet{fileToolSet, shellToolSet}),
	)
