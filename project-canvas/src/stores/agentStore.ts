...
import { Agent, AgentRole, AgentStatus, AgentFilter, AgentUpdateInput, UUID } from '../types/agent.types';
...
export const useAgentStore = create<AgentState>()(
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      assignTaskToAgent: (agentId, taskId) => set(
        (state) => ({
          agents: state.agents.map(agent =>
            agent.id === agentId
              ? {
      ...
      unassignTaskFromAgent: (agentId, taskId) => set(
        (state) => ({
          agents: state.agents.map(agent =>
            agent.id === agentId
              ? {
      ...
      getAgentsByRole: (role) => get().agents.filter(agent => agent.role === role),
      
      getAgentsByStatus: (status) => get().agents.filter(agent => agent.status === status),
      
      getAvailableAgents: () => {
        return get().agents.filter(agent => 
          agent.status === AgentStatus.ONLINE || agent.status === AgentStatus.IDLE
        );
      },
