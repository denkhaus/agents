...
func TestEntRepositoryInterface(t *testing.T) {
	...
	// Create repository
	repo, err := NewRepository(WithDatabaseURL(databaseURL), WithAutoMigrate(true))
	...
	t.Run("Task Operations", func(t *testing.T) {
		...
		// Test UpdateTask
		retrievedTask.State = shared.TaskStateInProgress
