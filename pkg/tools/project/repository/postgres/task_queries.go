...
import (
	...
	task "github.com/denkhaus/agents/pkg/tools/project/repository/postgres/ent/task"
...
func (r *postgresRepository) ListTasks(ctx context.Context, filter shared.TaskFilter) ([]*shared.Task, error) {
	...
	tasks := make([]*shared.Task, len(entTasks))
...
func (r *postgresRepository) GetTasksByProject(ctx context.Context, projectID uuid.UUID) ([]*shared.Task, error) {
...
func (r *postgresRepository) GetTasksByParent(ctx context.Context, parentID uuid.UUID) ([]*shared.Task, error) {
	entTasks, err := r.client.Task.Query().
...
func (r *postgresRepository) GetRootTasks(ctx context.Context, projectID uuid.UUID) ([]*shared.Task, error) {
	entTasks, err := r.client.Task.Query().
		Where(
...
func (r *postgresRepository) GetParentTask(ctx context.Context, taskID uuid.UUID) (*shared.Task, error) {
...
func (r *postgresRepository) GetProjectProgress(ctx context.Context, projectID uuid.UUID) (*shared.ProjectProgress, error) {
	...
	taskCounts := make(map[shared.TaskState]int)

	states := []shared.TaskState{
		shared.TaskStatePending,
		shared.TaskStateInProgress,
		shared.TaskStateCompleted,
		shared.TaskStateBlocked,
		shared.TaskStateCancelled,
	}
	...
	progress := 0.0
	if totalTasks > 0 {
		progress = float64(taskCounts[shared.TaskStateCompleted]) / float64(totalTasks) * 100.0
	}

	return &shared.ProjectProgress{
		ProjectID:       projectID,
		TotalTasks:      totalTasks,
		CompletedTasks:  taskCounts[shared.TaskStateCompleted],
		InProgressTasks: taskCounts[shared.TaskStateInProgress],
		PendingTasks:    taskCounts[shared.TaskStatePending],
		BlockedTasks:    taskCounts[shared.TaskStateBlocked],
		CancelledTasks:  taskCounts[shared.TaskStateCancelled],
