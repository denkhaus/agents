...
func entProjectToProject(ep *ent.Project) *shared.Project {
...
func projectToEntProjectCreate(p *shared.Project, client *ent.Client) *ent.ProjectCreate {
...
func entTaskToTask(et *ent.Task) *shared.Task {
	domainTask := &shared.Task{
		ID:          et.ID,
		ProjectID:   et.ProjectID,
		Title:       et.Title,
		Description: et.Description,
		State:       shared.TaskState(et.State),
...
func taskToEntTaskCreate(t *shared.Task, client *ent.Client) *ent.TaskCreate {
...
func taskToEntTaskUpdate(t *shared.Task, update *ent.TaskUpdateOne) *ent.TaskUpdateOne {
	update = update.
		SetTitle(t.Title).
		SetDescription(t.Description).
		SetState(task.State(t.State)).
...
func entProjectsToProjects(entProjects []*ent.Project) []*shared.Project {
	projects := make([]*shared.Project, len(entProjects))
...
func entTasksToTasks(entTasks []*ent.Task) []*shared.Task {
...
func filterMatchesTaskFilter(task *ent.Task, filter shared.TaskFilter) bool {
	...
	if filter.State != nil && shared.TaskState(task.State) != *filter.State {
		return false
	}
