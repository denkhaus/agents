...
// ValidateSchema validates data against a given schema (as a map[string]interface{}).
func ValidateSchema(data interface{}, schema map[string]interface{}) error {
	...
	processedData, err := resolveMethodsInStruct(data)
	if err != nil {
		return fmt.Errorf("failed to resolve methods in data: %w", err)
	}
	...
	if !result.Valid() {
		...
		return fmt.Errorf("data is not valid: %s", strings.Join(errors, "; "))
	}
